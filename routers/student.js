...
student.get('/session', requireAuth, async (request, response) => {
  let _id = request._id;
  try{
    let document = await sessionDB.findOne({_id: ObjectId(_id)});
    ...
    //exam in session, approve continuation of exam
      ...
      let doc = await answersDB.findOne({_id: ObjectId(_id)})
...
student.get('/login-status', requireAuth, async (request, response) => {
  ...
  try{
    ...
    let document = await sessionTrackerDB.findOne({name: name, session_id: request.cookies['AuthToken']});
...
student.get('/start-exam', async (request, response) => {
  ...
  let doc = await sessionDB.findOne({_id: ObjectId(_id)})
